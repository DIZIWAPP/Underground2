...
{
    CGPoint panStartingPosition;
    UICollectionViewCell *draggedCell;
}
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)gestureRecognizer {
    switch(gestureRecognizer.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            draggedCell = collectionViewCell;
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            CGPoint endPosition = [gestureRecognizer locationInView:gestureRecognizer.view];
            
            if ([self shouldDeleteStarting:panStartingPosition toPoint:endPosition])
            {
                if ([self.dataSource respondsToSelector:@selector(deleteCell:)])
                    [self.dataSource deleteCell:draggedCell];
                
                self.selectedItemIndexPath = nil;
                self.currentViewCenter = CGPointZero;
                
                [UIView animateWithDuration:0.3 animations:^{
                    self.currentView.transform = CGAffineTransformMakeScale(0, 0);
                } completion:^(BOOL finished) {
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gestureRecognizer {
    switch (gestureRecognizer.state) {
        case UIGestureRecognizerStateBegan:
        {
            panStartingPosition = [gestureRecognizer locationInView:gestureRecognizer.view];
        }
            break;
...
-(BOOL)shouldDeleteStarting:(CGPoint)point1 toPoint:(CGPoint)point2
{
    return (ABS(point1.y - point2.y) > 40) ? YES : NO;
}
